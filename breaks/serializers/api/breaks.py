...
class BreakMeCreateSerializer(InfoModelSerializer):
    """Сериализатор создания моего обеденного перерыва."""
    ...
    def validate(self, attrs):
        ...
        if attrs['break_start'] < replacement.break_start:
            raise ParseError(
<<<<<<< HEAD
                'Время начала не должно быть меньше времени, данного в смене.'
            )
        if attrs['break_end'] > replacement.break_end:
            raise ParseError(
                'Время окончания не должно быть больше времени, данного в '
=======
...
<<<<<<< HEAD
    """Сериализатор обновления моего обеденного перерыва."""
...
=======
    ...
    def validate(self, attrs):
        ...
        if 'break_start' in attrs and 'break_end' in attrs:
            ...
            if attrs['break_end'] > replacement.break_end:
                raise ParseError(
                    'Время окончания не должно быть больше времени, указанного'
                    ' в смене.'
                )
            if attrs['break_start'] >= attrs['break_end']:
                raise ParseError(
