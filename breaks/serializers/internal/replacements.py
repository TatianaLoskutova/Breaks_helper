...
class ReplacementPersonalStatsSerializer(ExtendedModelSerializer):
    ...
    def get_time_until_break(self, instance):
        if not instance:
            return None
        break_obj = instance.breaks.filter(
        ...
        break_minutes = (
            break_obj.break_start.hour * 60 + break_obj.break_start.minute
        )
        ...
        result = (
            f'{delta_hours // 10}{delta_hours % 10}: '
            f'{delta_minutes // 10}{delta_minutes % 10}'
        )
        return result
